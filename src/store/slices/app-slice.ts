...
export const loadAppDetails = createAsyncThunk(
    ...
    async ({ networkID, provider }: ILoadAppDetails) => {
        ...
        const circulatingBalance = (await timeContract.balanceOf(addresses.STAKING_ADDRESS)) / Math.pow(10, 9);
        ...
        const wBondsBalancesPromises = allBonds.map(bond => bond.getBalanceOf(networkID, timeContract));
        const wBondsBalances = await Promise.all(wBondsBalancesPromises);
        const wBondsBalance = wBondsBalances.reduce((WAmount0, WAmount1) => WAmount0 + WAmount1, 0);
        ...
        const stakingRate = circSupply / (totalSupply - wBondsBalance - timeAmount - (circulatingBalance - circSupply));
